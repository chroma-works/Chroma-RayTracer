...
namespace CHR
{
	...
	public:
		...
		glm::vec3 SampleLightDirection(const glm::vec3 isect_pos)
		{
			...
			float t_max = asinf(1/glm::length(obj_isect_pos));

			float chi_1 = CHR_UTILS::RandFloat(), chi_2 = CHR_UTILS::RandFloat();
			float theta = acosf(1.0f - chi_1 + chi_1 * cos(t_max)), phi = 2 * CHR_UTILS::PI * chi_2;
		...
		glm::vec3 RadianceAt(const glm::vec3 isect_pos, const glm::vec3 l_vec)
		{
			...
			return m_inten / (d * d) * (float)CHR_UTILS::PI * 2.0f * (1.0f - cos(t_max));
		}
